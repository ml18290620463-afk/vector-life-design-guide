...
export const Editor: React.FC<EditorProps> = ({
  ...
  const updateAiAnswer = (key: keyof typeof aiAnswers, value: string) => {
    setAiAnswers((current) => ({ ...current, [key]: value }));
  };

  const applyAiAssistDraft = () => {
    const scene = aiAnswers.scene.trim();
    const decision = aiAnswers.decision.trim();
    const feeling = aiAnswers.feeling.trim();
    const remember = aiAnswers.remember.trim();
    ...
    const lines =
      language === 'zh'
        ? [
            '## 核心叙事',
            scene ? `这件事发生在：${scene}` : '',
            decision ? `我当时的决定 / 行动是：${decision}` : '',
            feeling ? `事后最明显的感受是：${feeling}` : '',
            remember ? `以后看到这条记录，我想记住：${remember}` : '',
            existing ? ['', '## 原始内容', existing] : '',
          ]
        : [
            '## Core narrative',
            scene ? `Scene: ${scene}` : '',
            decision ? `Decision / action: ${decision}` : '',
            feeling ? `Strongest feeling afterward: ${feeling}` : '',
            remember ? `What I want to remember: ${remember}` : '',
            existing ? ['', '## Original note', existing] : '',
          ];
    const draft = lines.flat().filter(Boolean).join('\n');
    if (!draft) return;
    setContent(draft.slice(0, 2000));
  ...
  const handleSave = async () => {
    const normalizedTitle =
      ...
      content
        .trim()
        .replace(/\s+/g, ' ')
        .slice(0, language === 'zh' ? 18 : 42) ||
      (language === 'zh' ? '此刻的想法' : 'Untitled moment');

    if (!content.trim()) return;

    // VALIDATION: Check for duplicate titles
    if (existingTitles.some((t) => t.toLowerCase() === normalizedTitle.toLowerCase())) {
  ...
  return (
    ...
    >
      <AnimatePresence>
        {isSaving && (
          <motion.div
            className="fixed inset-0 z-[9998] flex items-center justify-center overflow-hidden bg-[#02050b]/92 backdrop-blur-xl"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
      ...
      >
        ...
        <div className="vector-editor-ai-menu">
          <button
            type="button"
            className={`vector-editor-ai-trigger ${aiAssistUsed ? 'vector-editor-ai-trigger--used' : ''}`}
            onClick={() => setShowAiAssist((value) => !value)}
            aria-expanded={showAiAssist}
            aria-label={language === 'zh' ? 'AI 帮我梳理' : 'AI help me structure'}
            title={language === 'zh' ? 'AI 帮我梳理' : 'AI help me structure'}
          >
            <Sparkles className="w-5 h-5" />
          </button>
          {showAiAssist && (
            <div className="vector-editor-ai-popover" role="dialog" aria-label={language === 'zh' ? 'AI 帮我梳理' : 'AI assist'}>
              <div className="vector-editor-ai-copy">
                {language === 'zh' ? '说不清也没关系，我只帮你问出来。' : 'If it is blurry, I will only help you get it out.'}
              </div>
              {(
                [
                  {
                    key: 'scene',
                    label: language === 'zh' ? '这件事大概发生在什么场景？' : 'What was the scene?',
                  },
                  {
                    key: 'decision',
                    label: language === 'zh' ? '你当时做了什么决定？' : 'What did you decide or do?',
                  },
                  {
                    key: 'feeling',
                    label: language === 'zh' ? '事后最明显的感受是什么？' : 'What stood out afterward?',
                  },
                  {
                    key: 'remember',
                    label: language === 'zh' ? '以后看到这条记录想记住什么？' : 'What should this record keep?',
                  },
                ] as const
              ).map((item) => (
                <label key={item.key}>
                  ...
                  <input
                    value={aiAnswers[item.key]}
                    onChange={(event) => updateAiAnswer(item.key, event.target.value)}
                    maxLength={160}
                  />
                </label>
              ))}
              <div className="vector-editor-ai-actions">
                <button type="button" onClick={() => setShowAiAssist(false)}>
                  {language === 'zh' ? '取消' : 'Cancel'}
                </button>
                <button type="button" onClick={applyAiAssistDraft}>
                  {language === 'zh' ? '生成草稿' : 'Make draft'}
                </button>
              </div>
            </div>
          )}
