...
export const SpaceTimeBackground: React.FC<SpaceTimeBackgroundProps> = ({ theme = 'dark' }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`absolute top-[-20%] left-[-10%] w-[80vw] h-[80vw] rounded-full blur-[120px] ${theme === 'light' ? 'bg-cyan-200/20' : 'bg-indigo-800/12'}`}
        ></div>
        <div
          className={`absolute bottom-[-10%] right-[-10%] w-[80vw] h-[80vw] rounded-full blur-[120px] ${theme === 'light' ? 'bg-blue-200/20' : 'bg-cyan-800/12'}`}
        ></div>

        {/* Stars */}
        <div
          className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-[0.12] z-0"
          style={{
        ...
        ></div>
        <div
          className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-[0.07] z-0"
          style={{
      ...
      <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
        <div className="relative top-[25%] w-[100vw] h-[100vw] flex items-center justify-center pointer-events-none">
          <div
            className={`absolute w-12 h-12 rounded-full z-30 transition-colors ${theme === 'light' ? 'bg-cyan-600/10' : 'bg-cyan-400/10 shadow-glow-cyan-neon-bright'}`}
          ></div>
        </div>
      </div>
      ...
      <div
        className={`absolute inset-0 flex justify-around overflow-hidden mix-blend-screen transition-opacity duration-700 ${theme === 'light' ? 'opacity-10' : 'opacity-34'}`}
      >
      ...
      <div className="fixed inset-0 pointer-events-none z-10">
        {/* Corner Brackets */}
        <div
          className={`absolute top-10 left-10 w-24 h-24 border-t-2 border-l-2 ${theme === 'light' ? 'border-cyan-200/50' : 'border-cyan-400/18'}`}
        />
        <div
          className={`absolute top-10 right-10 w-24 h-24 border-t-2 border-r-2 ${theme === 'light' ? 'border-cyan-200/50' : 'border-cyan-400/18'}`}
        />
        <div
          className={`absolute bottom-10 left-10 w-24 h-24 border-b-2 border-l-2 ${theme === 'light' ? 'border-cyan-200/50' : 'border-cyan-400/18'}`}
        />
        <div
          className={`absolute bottom-10 right-10 w-24 h-24 border-b-2 border-r-2 ${theme === 'light' ? 'border-cyan-200/50' : 'border-cyan-400/18'}`}
        />
